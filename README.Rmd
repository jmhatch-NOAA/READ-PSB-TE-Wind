...
file_img = purrr::map(.x = wind_tib$file_name, .f = function(.x) here::here('data', 'cc_tagged_density_data', .x) %>% raster(.) %>% projectRaster(., crs = st_crs(4326)$proj4string))
...
prob_wea = purrr::map(.x = file_img, .f = function(.x) purrr::map_dbl(.x = raster::extract(.x, boem_wind %>% as_Spatial()), .f = function(.x) sum(exp(.x))))
...
prob_df %<>% mutate(wea = factor(wea, levels = c('MA', 'RI/MA', 'RI', 'NY', 'NY/NJ', 'NJ', 'DE', 'MD', 'VA', 'NC'))) %>% group_by(wea, month, month_abb) %>% summarize(prob = sum(prob))
cc_pres_plt = ggplot(data = prob_df, aes(x = month, y = prob)) + 
  ...
  theme_cowplot() + 
  labs(y = 'probability')
ggsave(filename = here('imgs', 'cc_presence_weas.png'), device = 'png', bg = 'white', plot = cc_pres_plt, width = 7, height = 6, units = 'in')
```
...
m = leaflet() %>% 
  ...
  addPolygons(data = boem_wind, fillColor = '#FF2A00', color = '#FF2A00', popup = ~ADDITIONAL_INFORMATION, group = 'WEAs') %>%
...
## save
saveWidget(m, file = here::here('docs', 'index.html'))
```
