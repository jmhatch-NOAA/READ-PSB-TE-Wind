...
boem_wind = boem_wea_outlines %>% 
  ...
  mutate(STATE = case_when(STATE == 'Delaware' ~ 'DE',
                           STATE == 'Maryland' ~ 'MD', 
                           STATE == 'Delaware/Maryland' ~ 'DE/MD',
                           STATE %in% c('Massachusetts', 'Massachussets', ' Massachussets') ~ 'MA',
                           STATE == 'New Jersey' ~ 'NJ',
                           STATE == 'New York' ~ 'NY',
...
## remove CA, LA/TX, and ME
boem_wind %<>% dplyr::filter(!STATE %in% c('CA', ' Louisanna/Texas', 'Maine'))
...
prob_df %<>% mutate(wea = factor(wea, levels = c('MA', 'RI/MA', 'RI', 'NY', 'NY/NJ', 'NJ', 'DE', 'DE/MD', 'MD', 'VA', 'NC'))) %>% group_by(wea, month, month_abb) %>% summarize(prob = sum(prob, na.rm = TRUE))
